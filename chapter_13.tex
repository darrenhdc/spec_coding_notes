...
\subsection*{六、Spec-Driven vs Vibe Coding 的核心差异}

\subsubsection*{1. 什么是 Vibe Coding}

Vibe Coding 是指：
\begin{itemize}
\item 从模糊想法直接开始写代码
\item 边写边改，没有明确的 spec 阶段
\item 依赖 AI 的"理解"和"判断"
\item 快速迭代，但容易迷失方向
\end{itemize}

\subsubsection*{2. 什么是 Spec-Driven Coding}

Spec-Driven Coding 是指：
\begin{itemize}
\item 先写明确的 spec（功能定义）
\item 再写 plan（技术方案）
\item 再写 tasks（可执行步骤）
\item 最后 implement（写代码）
\end{itemize}

每一步都有明确的 artifact，每一步都可以被审查、修改、回滚。

\subsubsection*{3. 什么时候用哪种方式}

\textbf{适合 Vibe Coding 的场景：}
\begin{itemize}
\item 快速原型验证
\item 学习新技术
\item 一次性脚本
...
\textbf{适合 Spec-Driven Coding 的场景：}
\begin{itemize}
\item 长期项目
\item 多人协作
\item 复杂系统
...
\textbf{总结：}Vibe Coding 是"快速但不稳定"，Spec-Driven Coding 是"稳定但不快速"。选择哪种方式取决于你的项目需求、团队规模、时间约束。对于重要的、需要长期维护的项目，Spec-Driven Coding 是更好的选择。
